...
def check_nonzero_masks(pred_root: Path, datasets: list[str]) -> list[str]:
    issues: list[str] = []
    for ds in datasets:
        masks = list_images(pred_root / ds / "masks")
        if not masks:
            continue
        has_nonzero = False
        for p in masks:
...
def main() -> None:
    ...
    issues.extend(check_nonzero_masks(pred_root=pred_root, datasets=mandatory))
